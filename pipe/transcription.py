...
class Transcriber:
    @staticmethod
    def transcribe(clean_audio_path: str) -> list:
    ...
    @classmethod
    def run(cls, clean_audio_path: str, transcription_path: str) -> list:
        if Path(transcription_path).exists():
            print("-> Found cached Transcription.")
            with open(transcription_path, 'r', encoding='utf-8') as f:
                return json.load(f)
                
        transcription_data = cls.transcribe(clean_audio_path)
